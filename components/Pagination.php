...
class Pagination
{
    ...
    public function get()
    {
        ...
        $html = '<ul class="pagination" style="display: flex;  list-style-type: none;margin: 0;padding: 10px;margin: 10px;">';
        # Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            # Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                $links .= '<li class="active" style=""><a href="#" style="font-weight: 600;text-decoration: none;color:white;margin: 10px; padding: 10px;background-color: red;">' . $page . '</a></li>';
            } else {
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        return
                '<li><a style="font-weight: 600;text-decoration: none;margin: 10px; padding: 8px;border: 2px solid #eaeaea;color: black;" href="' . $currentURI . $this->index . $page . '">' . $text . '</a></li>';
    }
